<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="style.css">
    <title>Cuccinare</title>
</head>
<body>
    <header>
        <nav>
            <div class="logo">
                <a href="index.html">
                    <svg id="logo-14" width="73" height="49" viewBox="0 0 73 49" fill="none" xmlns="http://www.w3.org/2000/svg">
                       <path d="M46.8676 24C46.8676 36.4264 36.794 46.5 24.3676 46.5C11.9413 46.5 1.86765 36.4264 1.86765 24C1.86765 11.5736 11.9413 1.5 24.3676 1.5C36.794 1.5 46.8676 11.5736 46.8676 24Z" class="ccustom" fill="#68DBFF"></path> <path d="M71.1324 24C71.1324 36.4264 61.1574 46.5 48.8529 46.5C36.5484 46.5 26.5735 36.4264 26.5735 24C26.5735 11.5736 36.5484 1.5 48.8529 1.5C61.1574 1.5 71.1324 11.5736 71.1324 24Z" class="ccompli1" fill="#FF7917"></path> <path d="M36.6705 42.8416C42.8109 38.8239 46.8676 31.8858 46.8676 24C46.8676 16.1144 42.8109 9.17614 36.6705 5.15854C30.5904 9.17614 26.5735 16.1144 26.5735 24C26.5735 31.8858 30.5904 38.8239 36.6705 42.8416Z" class="ccompli2" fill="#5D2C02"></path> </svg>
                    </svg>
                    Cuccinare
                </a>
            </div>
            <ul class="nav-links">
                <li><a href="productos.html">Productos</a></li>
                <li><a href="carrito.html">Carrito</a></li>
                <li><a href="contacto.html">Contacto</a></li>
            </ul>
            <div class="user-actions">
                <a href="login.html">Iniciar Sesión</a>
                <a href="registro.html">Registro</a>
            </div>
        </nav>
    </header>
    <main>
        <!-- Slider con imagen ilustrativa -->
        <section class="slider">
            <div class="slide">
                <div class="image-container">
                    <img src="https://www.todococinaperu.com/wp-content/uploads/2021/03/Utensilios-1-1024x576.jpg" alt="Cocina amigable 1">
                </div>
                <div class="image-container">
                    <img src="https://i0.wp.com/todoparasucocina.com/wp-content/uploads/2020/06/cropped-logo-3-Azul-020959-TAMA%C3%91O-2000-pxs-PNG.png?fit=2000%2C358&ssl=1" alt="Cocina amigable 2">
                </div>
            </div>
        </section>

        <!-- Sección de Categorías de Productos -->
        <section class="categories">
            <h2>Nuestras Categorías</h2>

            <!-- Categoría 1 -->
            <div class="Esenciales">
                <h3>Esenciales</h3>
                <img src="https://lecasaprofesional.com/wp-content/uploads/2021/08/web_41002730l_01-1.jpg" alt="guantes">
                <img src="https://lh4.googleusercontent.com/z1HQJdrWhjbLXWJfuakKSLhM_Zsl_lUeBYSVkGP3grxdwFgV5gUy1DmhQz9T5DB6cC84F0U6UmwO0rLbrHSN7RrozV7yeVtgx5QysrI18DgZB6F-Uye2MnhM1y_o96Q3G7GGVXnKRefwqk6KLw" alt="Organizador de fregadero">
                <img src="https://images.fravega.com/f300/bfacf64766263d43bb06e6ebcaf498b8.jpg.webp" alt="Abrelatas">
                <p>Indispensables en la cocina.</p>
                <a href="productos.html" class="ver-mas-esenciales">Ver más</a>
            </div>

            <!-- Categoría 2 -->
            <div class="Con Garantia">
                <h3>Con Garantía</h3>
                <div class="Con Garantia-images">
                    <img src="https://media.revistaad.es/photos/637b302c5760daf31075ca10/master/w_1600,c_limit/1366_2000.jpeg" alt="Envasadora al vacío">
                    <img src="https://media.revistaad.es/photos/637b32db8b774040c55f8853/master/w_1600,c_limit/6134wZZ8n+L._AC_SL1000_.jpg" alt="Sartén parrilla 3 en 1">
                    <img src="https://lh4.googleusercontent.com/any22HUCQEp4pPORWX5KYz-29mvYmK5pIQUVduW3OOdlTRv58d3OpLZ5hP3U6FI5j6Bkxgnf3tjVTDMp67QRBsaqpvH9UuiT-fcsuKvgZWJIluB3gb4IXM4J4XzswhoM5_1O3vIBquWN5DiI_w" alt="Dispensador de aceite">
                </div>
                <p>Hasta 12 Meses de prueba.</p>
                <a href="productos.html" class="ver-mas-Con-Garantia">Ver más</a>
            </div>

            <!-- Categoría 3 -->
            <div class="Ofertas">
                <h3>Ofertas</h3>
                <img src="https://lh3.googleusercontent.com/4zAqLthaPlIYZuSRMZr_2U7PFKt8IYxBFZR2g3lv02q0VNbDDlhu3tiBm0_YpdI34Spsd5RESc1Ikbb8aMfHoSLFL1aoIEeQmyyLOCjaBp65mgNYzEmnXVVh_71JJhJHGqf6VYgboHd7Lrkvtw" alt="Afilador de cuchillos y tijeras">
                <img src="https://lavozdelmuro.net/wp-content/uploads/2015/01/17_gadgets_creativos_cocina_1-1.jpg" alt="Cucharón para caldos y sopas del monstruo del Lago Nahuel Huapi">
                <img src="https://media.revistaad.es/photos/637b30901f22cbb2f17f181c/master/w_1600,c_limit/7171EvuIDFL._AC_SL1500_.jpg" alt="Tijera de hierbas">
                <img src="https://http2.mlstatic.com/D_NQ_NP_2X_620766-MLA46490127718_062021-F.webp" alt="Cortador de papas">
                <p>10% de descuento.</p>
                <a href="productos.html">Ver más</a>
            </div>

        </section>

        <!-- Sección de Publicidades -->
        <section class="ads">
            <!-- Publicidad visible en dispositivos medianos y grandes -->
            <div id="carouselExampleIndicators" class="carousel slide">
                <div class="carousel-indicators">
                  <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
                  <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="1" aria-label="Slide 2"></button>
                  <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="2" aria-label="Slide 3"></button>
                </div>
                <class="carousel-inner">
                  <div class="carousel-item active">
                    <img src="https://images.alphacoders.com/280/280233.jpg" class="d-block w-100" alt="logo coca">
                  </div>
                  <div class="carousel-item">
                    <img src="https://www.insights.la/wp-content/uploads/2016/01/Coca-Cola-15.jpg" class="d-block w-100" alt="Coca cola bis">
                  </div>
                  <div class="carousel-item">
                    <img src="https://vivo247.com/wp-content/uploads/2020/06/coca-cola-28062020.png" class="d-block w-100" alt="coca tres">
                  </div>
                 <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide="prev">
                  <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                  <span class="visually-hidden">Previous</span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide="next">
                  <span class="carousel-control-next-icon" aria-hidden="true"></span>
                  <span class="visually-hidden">Next</span>
                </button>
              </div>
        </section>
    </main>

    <footer>
        <div class="footer-column">
            <!-- Columna 1: Logo del sitio -->
            <img src="https://media.licdn.com/dms/image/C4D0BAQHxlXnOv6WZvQ/company-logo_200_200/0/1649095404038?e=2147483647&v=beta&t=GNNYk6CHo5aTvs0i0iPOwRunojMVeQtSile1ZmXtY3M" alt="Logo de tu sitio">
        </div>
        <div class="footer-column">
            <!-- Columna 2: Redes sociales -->
            <ul class="social-icons">
                <li><a href="error404.html"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 128 128">
                    <path fill="#659ec9" d="M84.9,48.1H70.7v-9.3c0-3.5,2.3-4.3,4-4.3c1.6,0,10.1,0,10.1,0V19.1L70.8,19C55.5,19,52,30.5,52,37.9v10.3h-8.9V64H52c0,20.4,0,45,0,45h18.7c0,0,0-24.8,0-45h12.6L84.9,48.1z"></path><path fill="#fff" d="M70.8,29l13.8,0.1v-10L70.8,19C55.5,19,52,30.5,52,37.9v10C52,40.5,55.5,29,70.8,29z"></path><path fill="#444b54" d="M70.7,112H52c-1.7,0-3-1.3-3-3V67h-5.9c-1.7,0-3-1.3-3-3s1.3-3,3-3H52c1.7,0,3,1.3,3,3v42h12.7V64c0-1.7,1.3-3,3-3h9.9l1-9.9H70.7c-1.7,0-3-1.3-3-3v-9.3c0-5.1,3.5-7.3,7-7.3h7.1V22l-10.9,0C66.1,22,55,23.5,55,37.9v10.3c0,1.7-1.3,3-3,3h-8.9c-1.7,0-3-1.3-3-3s1.3-3,3-3H49v-7.3C49,27.8,54.7,16,70.8,16l13.9,0.1c1.7,0,3,1.3,3,3v15.4c0,1.7-1.3,3-3,3H74.6c-0.6,0-1,0-1,1.3v6.3h11.2c0.8,0,1.7,0.4,2.2,1c0.6,0.6,0.8,1.5,0.8,2.3l-1.6,15.9c-0.2,1.5-1.4,2.7-3,2.7h-9.6v42C73.7,110.7,72.3,112,70.7,112z"></path><path fill="#444b54" d="M84.7 112H42c-1.7 0-3-1.3-3-3s1.3-3 3-3h42.7c1.7 0 3 1.3 3 3S86.3 112 84.7 112zM100.7 112c-.8 0-1.6-.3-2.1-.9-.6-.6-.9-1.3-.9-2.1 0-.2 0-.4.1-.6 0-.2.1-.4.2-.6.1-.2.2-.3.3-.5.1-.2.2-.3.4-.5 1.1-1.1 3.1-1.1 4.2 0 .6.6.9 1.3.9 2.1s-.3 1.6-.9 2.1C102.2 111.7 101.5 112 100.7 112z"></path>
                    </svg></a></li>
                <li><a href="error404.html"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 128 128">
                    <path fill="#76ecff" d="M100.2,42.5c-0.4,0.4-0.7,1-0.6,1.6c0.1,1.9,2.6,47.4-45.7,58.8c-0.2,0-19.4,4-36-4.2c7-0.2,17.1-2,24.1-9.9c0.5-0.6,0.7-1.4,0.3-2.2c-0.3-0.7-1-1.2-1.8-1.2c0,0,0,0,0,0c-0.1,0.1-10.6,0-15.5-10c1.9,0.1,4.3,0,6.3-0.9c0.8-0.4,1.3-1.2,1.2-2c-0.1-0.9-0.7-1.6-1.6-1.7c-0.5-0.1-12.3-2.7-13.9-15.5c1.8,0.8,4.1,1.4,6.3,1.1c0.8-0.1,1.4-0.7,1.6-1.4c0.2-0.8,0-1.6-0.6-2.1c-0.5-0.4-11.5-10.2-6.3-22.9c5.7,6,22.5,21.4,42.9,20.3c0.6,0,1.1-0.3,1.5-0.8c0.4-0.5,0.5-1.1,0.3-1.7c-0.4-1.5-0.6-3.1-0.6-4.7c0-10.4,8.3-18.9,18.6-18.9c5,0,9.7,2,13.2,5.6c0.4,0.4,0.9,0.6,1.4,0.6c2.1,0,5.6-0.2,9.6-1.8c-1.1,1.4-2.8,3.1-5.3,5.1c-0.7,0.5-1,1.5-0.6,2.3c0.3,0.8,1.2,1.3,2,1.3c0.5,0,2.9-0.2,5.6-0.8C105,37.9,103,39.9,100.2,42.5z"></path><path fill="#fff" d="M32.1 65.1c.8-.1 1.4-.7 1.6-1.4.2-.8 0-1.6-.6-2.1-.5-.4-11.5-10.2-6.3-22.9 5.7 6 22.5 21.4 42.9 20.3.6 0 1.1-.3 1.5-.8.4-.5.5-1.1.3-1.7-.4-1.5-.6-3.1-.6-4.7 0-10.4 8.3-18.9 18.6-18.9 3.3 0 14.2-3 15.3-4.4-4 1.6-7.5 1.8-9.6 1.8-.5 0-1-.2-1.4-.6-3.5-3.6-8.2-5.6-13.2-5.6C70.3 24.1 62 32.5 62 42.9c0 1.6.2 3.2.6 4.7.1.6 0 1.2-.3 1.7-.4.5-.9.8-1.5.8-20.4 1.2-37.2-14.3-42.9-20.3-5.2 12.6 5.8 22.4 6.3 22.9.6.5.8 1.3.6 2.1-.2.8-.9 1.3-1.6 1.4-2.3.3-4.5-.3-6.3-1.1 1.1 8.5 6.6 12.5 10.4 14.3-.7-1.6-1.2-3.3-1.5-5.4C27.5 64.8 29.8 65.5 32.1 65.1zM40.1 83.4c.8-.4 1.3-1.2 1.2-2-.1-.9-.7-1.6-1.6-1.7-.3-.1-5-1.1-8.8-4.9-1.8.7-4 .8-5.9.7 2.5 5.1 6.5 7.7 9.8 8.9C36.5 84.3 38.5 84.1 40.1 83.4zM50.8 97.7c.5-.6.7-1.4.3-2.2-.3-.7-1-1.2-1.8-1.2 0 0 0 0 0 0-.1 0-5 0-9.7-3.2-6.8 5.9-15.5 7.4-21.8 7.6 8.5 4.2 17.8 5.2 24.7 5.2C45.5 102.4 48.4 100.4 50.8 97.7z"></path><path fill="#444b54" d="M24.2 55.8c-.7 0-1.4-.2-2-.7-.5-.5-13.1-11.7-7.1-26.3.4-.9 1.2-1.6 2.2-1.8 1-.2 2 .1 2.7.9 16.7 17.5 32.3 19.5 39.3 19.4-.3-1.4-.4-2.8-.4-4.2 0-12 9.7-21.9 21.6-21.9 5.7 0 11 2.2 15 6.2 6.7-.4 10.7-3.7 10.8-3.7 1.3-1.1 3.2-.9 4.2.3 1.1 1.3.9 3.2-.3 4.2-.2.2-5.8 4.9-15 5.2-.1 0-.1 0-.2 0-1.3 0-2.6-.6-3.5-1.5-2.9-3-6.9-4.7-11.1-4.7C72 27.1 65 34.2 65 42.9c0 1.3.2 2.7.5 4 .4 1.5.1 3-.8 4.2-.9 1.2-2.3 2-3.8 2-18 1-33.2-9.9-41.4-17.4-.6 8.4 6.3 14.5 6.6 14.8 1.2 1.1 1.4 3 .3 4.2C25.9 55.4 25 55.8 24.2 55.8zM95.9 27.6C95.9 27.6 95.9 27.6 95.9 27.6 95.9 27.6 95.9 27.6 95.9 27.6zM99.6 47.1c-1 0-2-.5-2.6-1.5-.8-1.4-.3-3.3 1.1-4.1 5.4-3.1 11-10.2 11.1-10.3 1-1.3 2.9-1.5 4.2-.5 1.3 1 1.5 2.9.5 4.2-.3.3-6.4 8.1-12.8 11.8C100.6 46.9 100.1 47.1 99.6 47.1zM30.8 73.7c-.2 0-.4 0-.6-.1-.6-.1-14.4-3.2-16.3-18.1-.2-1.6 1-3.1 2.6-3.4 1.6-.2 3.1 1 3.4 2.6 1.4 10.7 11.2 12.9 11.6 13 1.6.3 2.7 1.9 2.3 3.6C33.4 72.8 32.2 73.7 30.8 73.7z"></path><path fill="#444b54" d="M40.5 88.5L40.5 88.5c-1.7.1-3.8-.4-5.1-.8-4.1-1.2-9.8-4-13.1-11-.7-1.5-.1-3.3 1.4-4 1.5-.7 3.3-.1 4 1.4 3.8 7.9 11.9 8.3 12.6 8.4 1.7-.1 3.2 1.3 3.2 3C43.5 87.1 42.1 88.5 40.5 88.5zM38.8 82.9C38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9zM38.8 82.9C38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9zM38.8 82.9C38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9zM38.8 82.9C38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9 38.8 82.9zM40.6 95c-.2 0-.4 0-.6-.1-.2 0-.4-.1-.6-.2-.2-.1-.4-.2-.5-.3-.2-.1-.3-.2-.5-.4-.1-.1-.3-.3-.4-.5-.1-.2-.2-.3-.3-.5-.1-.2-.1-.4-.2-.6 0-.2-.1-.4-.1-.6 0-.2 0-.4.1-.6 0-.2.1-.4.2-.6.1-.2.2-.4.3-.5.1-.2.2-.3.4-.5.1-.1.3-.3.5-.4.2-.1.3-.2.5-.3.2-.1.4-.1.6-.2.4-.1.8-.1 1.2 0 .2 0 .4.1.6.2.2.1.4.2.5.3.2.1.3.2.5.4s.3.3.4.5.2.3.3.5c.1.2.1.4.2.6 0 .2.1.4.1.6 0 .2 0 .4-.1.6 0 .2-.1.4-.2.6-.1.2-.2.4-.3.5s-.2.3-.4.5c-.1.1-.3.3-.5.4-.2.1-.3.2-.5.3-.2.1-.4.1-.6.2C41 94.9 40.8 95 40.6 95z"></path><g><path fill="#444b54" d="M17.9,101.7c-1.6,0-3-1.3-3-2.9c0-1.7,1.3-3,2.9-3.1c5.3-0.1,10.1-1.2,14.1-3.1c1.5-0.7,3.3-0.1,4,1.4c0.7,1.5,0.1,3.3-1.4,4C29.7,100.3,24.1,101.5,17.9,101.7C17.9,101.7,17.9,101.7,17.9,101.7z"></path></g><g><path fill="#444b54" d="M42.3,106.9c-3,0-6.4-0.2-10-0.7c-1.6-0.2-2.8-1.8-2.5-3.4c0.2-1.6,1.8-2.8,3.4-2.5c10.8,1.6,20-0.3,20.1-0.3C99,89.1,96.7,46.1,96.6,44.3c-0.1-1.7,1.1-3.1,2.8-3.2c1.7-0.1,3.1,1.1,3.2,2.8c0.1,2,2.7,49.9-48,61.9C54.2,105.9,49.3,106.9,42.3,106.9z"></path></g>
                    </svg></a></li>
                <li><a href="error404.html"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 128 128">
                    <path fill="#ff697b" d="M86.2,109H41.8C32,109,24,101,24,91.2V46.8C24,37,32,29,41.8,29h44.4C96,29,104,37,104,46.8v44.4C104,101,96,109,86.2,109z"></path><path fill="#fff" d="M24,46.8V58h80V46.8C104,37,96,29,86.2,29H41.8C32,29,24,37,24,46.8z"></path><path fill="#444b54" d="M86.2,112H41.8C30.3,112,21,102.7,21,91.2V46.8C21,35.3,30.3,26,41.8,26h44.4c11.5,0,20.8,9.3,20.8,20.8v44.4C107,102.7,97.7,112,86.2,112z M41.8,32C33.6,32,27,38.6,27,46.8v44.4c0,8.1,6.6,14.8,14.8,14.8h44.4c8.1,0,14.8-6.6,14.8-14.8V46.8c0-8.1-6.6-14.8-14.8-14.8H41.8z"></path><path fill="#fff" d="M64 54A15 15 0 1 0 64 84A15 15 0 1 0 64 54Z"></path><path fill="#444b54" d="M64,87c-9.9,0-18-8.1-18-18c0-1.7,1.3-3,3-3s3,1.3,3,3c0,6.6,5.4,12,12,12s12-5.4,12-12s-5.4-12-12-12c-1.7,0-3-1.3-3-3s1.3-3,3-3c9.9,0,18,8.1,18,18S73.9,87,64,87z"></path><path fill="#444b54" d="M53.8 60.9c-.2 0-.4 0-.6-.1s-.4-.1-.6-.2c-.2-.1-.4-.2-.5-.3-.2-.1-.3-.2-.5-.4-.1-.1-.3-.3-.4-.5-.1-.2-.2-.3-.3-.5-.1-.2-.1-.4-.2-.6 0-.2-.1-.4-.1-.6 0-.2 0-.4.1-.6 0-.2.1-.4.2-.6.1-.2.2-.4.3-.5.1-.2.2-.3.4-.5.1-.1.3-.3.5-.4.2-.1.3-.2.5-.3.2-.1.4-.1.6-.2.4-.1.8-.1 1.2 0 .2 0 .4.1.6.2.2.1.4.2.5.3.2.1.3.2.4.4.1.1.3.3.4.5.1.2.2.3.3.5.1.2.1.4.2.6 0 .2.1.4.1.6 0 .2 0 .4-.1.6 0 .2-.1.4-.2.6-.1.2-.2.4-.3.5-.1.2-.2.3-.4.5-.1.1-.3.3-.4.4-.2.1-.3.2-.5.3-.2.1-.4.1-.6.2S54 60.9 53.8 60.9zM86 43A4 4 0 1 0 86 51 4 4 0 1 0 86 43zM19 112.3c-1.6 0-3-1.3-3-3 0-1.7 1.3-3 3-3l90-.5c0 0 0 0 0 0 1.6 0 3 1.3 3 3 0 1.7-1.3 3-3 3L19 112.3C19 112.3 19 112.3 19 112.3zM124 111.8c-.8 0-1.6-.3-2.1-.9-.1-.1-.3-.3-.4-.5-.1-.2-.2-.3-.3-.5-.1-.2-.1-.4-.2-.6 0-.2-.1-.4-.1-.6 0-.2 0-.4.1-.6 0-.2.1-.4.2-.6.1-.2.2-.4.3-.5.1-.2.2-.3.4-.5.1-.1.3-.3.4-.4.2-.1.3-.2.5-.3.2-.1.4-.1.6-.2.4-.1.8-.1 1.2 0 .2 0 .4.1.6.2.2.1.4.2.5.3.2.1.3.2.4.4.6.6.9 1.3.9 2.1 0 .2 0 .4-.1.6 0 .2-.1.4-.2.6-.1.2-.2.4-.3.5-.1.2-.2.3-.4.5C125.6 111.5 124.8 111.8 124 111.8z"></path>
                    </svg></a></li>
            </ul>
        </div>
        <div class="footer-column">
            <!-- Columna 3: Información optativa -->
            <p>&copy; 2023 Cuccinare. Todos los derechos reservados.</p>
        </div>
    </footer>
</body>
</html>
</body>
</html>


